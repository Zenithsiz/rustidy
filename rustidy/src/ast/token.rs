...
pub macro decl_tokens(
	...
	$(
		...
		pub enum ${concat($TokenName, Error)} {
		...
		impl Parse for $TokenName {
			type Error = ${concat($TokenName, Error)};
			...
			fn parse_from(parser: &mut Parser) -> Result<Self, Self::Error> {
				...
				let whitespace = parser.parse::<Whitespace>().map_err(Self::Error::Whitespace)?;

				Ok(Self(token.into(), whitespace))
			}
		}
