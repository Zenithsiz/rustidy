...
pub struct Function {
	...
	#[format(before_with(expr = Format::prefix_ws_set_single, if = self.qualifiers.has_any()))]
	pub fn_:        token::Fn,
	#[parse(fatal)]
...
pub struct TypedSelf {
	pub mut_:  Option<token::Mut>,
	#[format(before_with(expr = Format::prefix_ws_set_single, if = self.mut_.is_some()))]
	pub self_: token::SelfLower,
	#[format(before_with = Format::prefix_ws_remove)]
	pub colon: token::Colon,
	#[format(before_with = Format::prefix_ws_set_single)]
